...
Allows locating input elements by the text of the associated or
...
##### [go_back](https://erikaris.github.io/rplaywright/)
...
##### [go_forward](https://erikaris.github.io/rplaywright/)
...
##### [remove_locator_handler](https://erikaris.github.io/rplaywright/)
...
##### [set_extra_h_t_t_p_headers](https://erikaris.github.io/rplaywright/)
...
##### [workers](https://erikaris.github.io/rplaywright/)
...
If the element is inside the element that has an associated control,
...
sequence of events are emitted by Page:

- page.on(‘request’) emitted when the request is issued by the page.
- page.on(‘response’) emitted when/if the response status and headers
  are received for the request.
- page.on(‘requestfinished’) emitted when the response body is
  downloaded and the request is complete.

If request fails at some point, then instead of ‘requestfinished’ event
(and possibly instead of ‘response’ event), the page.on(‘requestfailed’)
event is emitted.
...
headers, including cookie-related ones. You can use request.allHeaders()
...
with multiple entries, such as Set-Cookie, appear in the array multiple
times.
...
Some navigation requests are issued before the corresponding frame is
...
When the response is application/x-www-form-urlencoded then a key/value
object of the values will be returned. Otherwise it will be parsed as
JSON.
...
Request object. The two requests are connected by redirectedFrom() and
redirectedTo() methods. When multiple server redirects has happened, it
is possible to construct the whole redirect chain by repeatedly calling
redirectedFrom().
...
available when request finishes. Find more information at Resource
Timing API.
